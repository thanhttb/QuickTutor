...
@section('content')
    ...
    <form class="form-horizontal" action="/editProfile/save/{{ $profile->id }}" method="POST">
        ...
        {{ csrf_field() }}
        <div class="col-sm-6">
            <h2 style="color:blue"><b>Thông tin cá nhân </b></h2>
            <hr>
            <div class="form-group">
              <label class="control-label col-sm-3" for="name" required>Họ và Tên</label>
              <div class="col-sm-9">
                <input type="text" class="form-control" name="name" id="name" value= "{{$profile->name}}" placeholder="{{$profile->name}}">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <input type="email" class="form-control" name="email" id="email" value= "{{$profile->email}}" placeholder="{{$profile->email}}">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <input type="tel" class="form-control" name="phone1" id="phone1" value= "{{$profile->phone1}}" placeholder="{{$profile->phone1}}">
              </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="date" class="form-control" name = "birthDay" id="birthDay" value= {{$profile->birthDay}}>
                </div>
            </div>
        ...
        <div class="col-sm-6">
            <h2 style="color:blue"><b>Thông tin giảng dạy:</b></h2>
            <hr>

            <div class="form-group">
                <label class="control-label col-sm-3" for="subject" >Môn học</label>
                <div class="col-sm-9">
                    <select class="js-example-basic-multiple js-states form-control" id="subjects" name="subjects[]" multiple="multiple">
                          @foreach($subjects as $subject)
                              <option value="{{$subject->id}}" @foreach($profile->subjects as $s)
                                    @if($subject->id == $s->id)
                                        selected="selected"
                                    @endif
                              @endforeach>
